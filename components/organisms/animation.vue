...
export default {
  ...
  methods: {
    ...
    scroll(progress) {
      this.$data.count = Math.floor(
        (progress * (this.$props.options.number - 1)) / 100
      );
    },
  },
  watch: {
    "$data.count": function () {
      this.$data.context.clearRect(
        0,
        0,
        this.$data.canvas.width,
        this.$data.canvas.height
      );
      this.$data.context.drawImage(
        this.$data.images[this.$data.count],
        0,
        0,
        this.$data.canvas.width,
        this.$data.canvas.height
  ...
  mounted() {
    for (let i = 0; i < this.$props.options.number; i++) {
      ...
      image.src = require(`~/assets/datas/images/${this.$props.options.name}-${
        i + 1
      }.png`);
